...
| __Integrations__ | AWS SDK, OpenTofu, AWS CLI                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
The OpenTofu configuration file now randomly generates names for the bucket, in order to avoid conflicts
at a global scale on AWS. This name shall be written out to a properties file, which the app will pick up
and use for the S3 client. Furthermore, the name is also passed as an environment variable to the Lambda function by OpenTofu,
...
- [OpenTofu](https://opentofu.org/docs/intro/install/) (+ Python pip for [tflocal](https://pypi.org/project/terraform-local/)) for creating AWS & LocalStack resources
- [npm](https://docs.npmjs.com/downloading-and-installing-node-js-and-npm) - for running the frontend app

#### What it does

*shipment-list-demo* is a Spring Boot application dealing with CRUD operations one can
execute
...
The OpenTofu configuration file will create the needed S3 bucket, the DynamoDB `shipment` table and populate it with some
...
### Creating resources - running OpenTofu

Make sure you have OpenTofu [installed](https://opentofu.org/docs/intro/install/)
...
Before moving on, make sure you clean up your AWS resources by running (also in the `opentofu` folder):
...
the `terraform`/`tofu` command line client. `tflocal` takes care of automatically configuring the local
service
endpoints, which allows you to easily deploy your unmodified OpenTofu scripts against LocalStack.

#### Note

`tflocal` supports both Terraform and OpenTofu. By default, it uses the Terraform binary in your system.
To use OpenTofu, all you need to do is customize the `TF_CMD` environment variable to use the `tofu` binary:
...
Once installed, the `tflocal` command should be available, with the same interface as the tofu
...
From here on, it's the same as using AWS. In the `opentofu` folder, run the `cleanup` script
...
variables, since the bucket name is generated and passed by OpenTofu.
